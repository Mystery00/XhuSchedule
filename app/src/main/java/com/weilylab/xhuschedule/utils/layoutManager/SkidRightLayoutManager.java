package com.weilylab.xhuschedule.utils.layoutManager;
...
public class SkidRightLayoutManager extends RecyclerView.LayoutManager {
	...
	public void fill(RecyclerView.Recycler recycler) {
		...
		ArrayList<ItemViewInfo> layoutInfoList = new ArrayList<>();
		...
		if (bottomItemPosition < mItemCount) {
			...
			layoutInfoList.add(new ItemViewInfo(start, 1.0f,
		...
		int layoutCount = layoutInfoList.size();
		...
		for (int i = 0; i < layoutCount; i++) {
			fillChild(recycler.getViewForPosition(convert2AdapterPosition(startPos + i)), layoutInfoList
					.get(i));
		}
	}
